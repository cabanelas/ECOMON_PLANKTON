...
# check
SNE1 %>% filter(taxa == "megan" & season == "spring") %>%
...
# check
SNE2 %>% filter(taxa == "euphkr") %>%
  ggplot(aes(x = Date, y = Anomaly_yr)) +
  geom_point()

# any NAs?
SNE2 %>% filter(taxa == "nemame" & year == 2010) %>%
  ggplot(aes(x = Date, y = Anomaly_yr)) +
  geom_point() +
  geom_line()
#yes
...
##################################################################
## Data
#amoz <- read.csv("raw/amon.us.short.csv")
...
#amozl <- amozl %>% filter(yr >= 1977 & yr <= 2020)

#amozl$Date <- zoo::as.yearmon(paste(amozl$yr, amozl$month), "%Y %b")
#amozl$Date <- format(amozl$Date, '%Y-%m-%01')
#amozl$Date <- as.Date(amozl$Date, '%Y-%m-%d')

#ggplot(data = amozl, aes(x = Date, y = amo)) +
#  geom_line() +
#  geom_point() +
#  theme_classic() +
#  geom_hline(yintercept = 0)

#amozl$monthNum <- match(amozl$month, month.abb)
...
#amowinter <- ggplot(data = amozl1, aes(x = Date, y = amo)) +
#  geom_line() +
#  geom_point() +
#  theme_classic() +
#  geom_hline(yintercept = 0)
...
#amowingrou <- amowingrou %>%
#  mutate(col = factor(ifelse(amos > 0, 2, 1)))

#amowingrouP <- ggplot(data = amowingrou, aes(x = Date, y = amos, color = col, fill = col)) + #need to add fill argument due to pixel issue 
#  geom_bar(stat = "identity", 
#           position = "dodge",
#           show.legend=FALSE) +
#  scale_fill_manual(values=c("blue", "red"))  +
...
#megw <- SNE2a %>% 
#  filter(taxa == "megan" & season == "winter" & year < 2021) %>%
#  ungroup()

#meganAnomw <- ggplot(data = megw, aes(x = Date, y = Anomaly_yr)) +
#  geom_line() +
#  geom_point() +
#  theme_classic() + 
...
#  labs(y='year') +
#  scale_x_discrete(labels = function(x) str_wrap(x, width = 8)) +
#  theme(axis.text.x = element_text(angle = 90)) ##WRONG ORRR????
...
#plot1w <- plot_grid(meganAnomw, amoAnomw, amoMegw, nrow=3)
...
meganAnomw1 <- ggplot(data = megw1, aes(x = Date, y = Anomaly_yr)) +
  geom_line() +
  geom_point() +
  theme_classic() + 
  ggtitle("SNE - Meganyctiphanes norvegica - winter") +
...
meggroupwP <- ggplot(data = meggroupw, aes(x = Date, y = sumA, color = col, fill = col)) + #need to add fill argument due to pixel issue 
  geom_bar(stat = "identity", 
           position = "dodge",
           show.legend=FALSE)  +
  geom_hline(yintercept = 0) +
...
#meganAnoms <- ggplot(data = megsum, aes(x = Date, y = Anomaly_yr)) +
#  geom_line() +
#  geom_point() +
#  theme_classic() + 
...
#            linewidth = 1.2) +
#  geom_line(data = megsum, aes(x = Date, y = Anomaly_yr),
#            linewidth = 0.8) +
#  geom_point(data = amos, aes(x = Date, y = amo), color = "red") +
#  geom_point(data = megsum, aes(x = Date, y = Anomaly_yr)) +
#  theme_classic() +
#  geom_hline(yintercept = 0)

#plot_grid(meganAnoms, amoAnoms, amoMegs, nrow=3)

##without interpolating abundance data
megsum1 <- SNE2 %>% 
  filter(taxa == "megan" & season == "summer")

meganAnomsum1 <- ggplot(data = megsum1, aes(x = Date, y = Anomaly_yr)) +
  geom_line() +
  geom_point() +
  theme_classic() + 
  ggtitle("SNE - Meganyctiphanes norvegica - summer") +
...
amoMegsu1 <- ggplot() +
  geom_line(data = amos, aes(x = Date, y = amo), color = "red",
            linewidth = 1.2) +
  geom_line(data = megsum1, aes(x = Date, y = Anomaly_yr),
            linewidth = 0.8) +
  geom_point(data = amos, aes(x = Date, y = amo), color = "red") +
  geom_point(data = megsum1, aes(x = Date, y = Anomaly_yr)) +
  theme_classic() +
  geom_hline(yintercept = 0)
...
meggroupsuP <- ggplot(data = meggroups, aes(x = Date, y = sumA, color = col, 
                                           fill = col)) + #need to add fill argument due to pixel issue 
  geom_bar(stat = "identity", 
           position = "dodge",
           show.legend=FALSE)  +
